...
public class CustomAdapter extends ArrayAdapter<CustomItemsCat> {
    public CustomAdapter(@NonNull Context context, ArrayList<CustomItemsCat> customList) {
    ...
    public View customView(int position, @Nullable View convertView, @NonNull ViewGroup parent) {
        if (convertView == null) {
            convertView = LayoutInflater.from(getContext()).inflate(R.layout.custom_spinner_categ1, parent, false);
        }
        CustomItemsCat items = getItem(position);
